...
import tech.simter.file.impl.TestHelper.randomAttachmentId
...
class PackageAttachmentsMethodTest @Autowired constructor(
  ...
  fun packageAttachments() {
    ...
    val dtos = listOf(
      randomAttachmentZipInfo(zipPath = "zip-path1", physicalPath = "path1", type = ":d"),
      randomAttachmentZipInfo(zipPath = "zip-path1/zip-file1", physicalPath = "path1/file1.txt", type = "txt"),
      randomAttachmentZipInfo(zipPath = "zip-path1/zip-path2", physicalPath = "path1/path2", type = ":d"),
      randomAttachmentZipInfo(zipPath = "file2", physicalPath = "file2.txt", type = "txt"),
      randomAttachmentZipInfo(zipPath = "zip-file1", physicalPath = "path1/file1.txt", type = "txt")
    )
    ...
    clearMocks(dao)
    clearMocks(service)
    verifyPackage(listOf(dtos[4].copy(origin = dtos[4].terminus)), "zip-file1.txt.zip")
    ...
    clearMocks(dao)
    clearMocks(service)
    verifyPackage(dtos.subList(0, 3).map { it.copy(origin = dtos[0].terminus) }, "zip-path1.zip")
  }

  private fun randomAttachmentZipInfo(zipPath: String, physicalPath: String, type: String): AttachmentZipInfoImpl {
    return AttachmentZipInfoImpl(
      id = randomAttachmentId(),
      terminus = randomAttachmentId(),
      zipPath = zipPath,
  ...
  private fun verifyPackage(dtos: List<AttachmentZipInfo>, zipName: String) {
